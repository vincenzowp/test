...
;(function($) {
	jQuery.fn.fancyNews = function(arg) {
		...
		var $elem, newsContainers, $newsList, $nav, previewSize, slideAmount, slideInt, contents = [], topIndex = 0, listItemHeight, jsApiRequested = false;
			...
			function _loadFromXml() {
				...
				if(jsApiRequested == false) {
					//alert('Please include the Google JS API in a script tag: https://www.google.com/jsapi');
					$.getScript('https://www.google.com/jsapi', function(data) {
						google.load("feeds", "1", {"callback" : _feedApiLoaded});
			...
			function _feedApiLoaded() {

				var timestamp = new Date(),
					feed = new google.feeds.Feed(options.feed);

				feed.setNumEntries(options.numOfEntries);
			...
			function _appendNewsItem(title, date, excerpt, content, image) {
				...
				for(var j=0; j < 4; ++j) {
					var lastIndex = currentString.lastIndexOf(' ');
